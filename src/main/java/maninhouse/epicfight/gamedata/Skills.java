...
import maninhouse.epicfight.skill.*;
...
public class Skills {
	...
	public static Skill findSkill(String skillName) {
		...
		if (MODIFIABLE_SKILLS.containsKey(rl)) {
			return MODIFIABLE_SKILLS.get(rl);
		} else {
			throw new IllegalArgumentException("Can't find skill name " + skillName);
		}
	}
	
	public static Collection<Skill> getSkillCollection() {
	...
	public static Skill registerSkill(Skill skill, boolean appearInCreativeTab) {
		if (appearInCreativeTab) {
			...
			if (MODIFIABLE_SKILLS.containsKey(rl)) {
				...
				MODIFIABLE_SKILLS.put(rl, skill);
			}
		}
