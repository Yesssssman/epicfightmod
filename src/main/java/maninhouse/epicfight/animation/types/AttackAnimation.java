...
import maninhouse.epicfight.entity.eventlistener.AttackEndEvent;
import maninhouse.epicfight.entity.eventlistener.DealtDamageEvent;
...
public class AttackAnimation extends ActionAnimation {
	...
	public AttackAnimation(int id, float convertTime, float antic, float preDelay, float contact, float recovery, boolean affectY, @Nullable Collider collider,
		...
		this(id, convertTime, true, affectY, path, new Phase(antic, preDelay, contact, recovery, index, collider));
	}
	
	public AttackAnimation(int id, float convertTime, float antic, float preDelay, float contact, float recovery, boolean breakMovement, boolean affectY, @Nullable Collider collider,
			String index, String path) {
		this(id, convertTime, breakMovement, affectY, path, new Phase(antic, preDelay, contact, recovery, index, collider));
	}
	...
	public AttackAnimation(int id, float convertTime, float antic, float preDelay, float contact, float recovery, boolean affectY, Hand hand, @Nullable Collider collider,
			String index, String path) {
		this(id, convertTime, true, affectY, path, new Phase(antic, preDelay, contact, recovery, hand, index, collider));
	}
	
	public AttackAnimation(int id, float convertTime, boolean affectY, String path, Phase... phases) {
		this(id, convertTime, true, affectY, path, phases);
	}
	
	public AttackAnimation(int id, float convertTime, boolean breakMovement, boolean affectY, String path, Phase... phases) {
		super(id, convertTime, breakMovement, affectY, path);
		this.phases = phases;
	}
	...
	protected Pose getRawPose(LivingData<?> entitydata, float time) {
		Pose pose = super.getRawPose(entitydata, time);
		this.getProperty(AttackAnimationProperty.DIRECTIONAL).ifPresent((b)->{
	...
	public BindingOperation getBindingOperation(String joint) {
		BindingOperation bindOperation = super.getBindingOperation(joint);
		if ((joint.equals("Chest") || joint.equals("Head")) && bindOperation == BindingOption.DEFAULT) {
			bindOperation = BindingOption.DYNAMIC_TRANSFORM;
		}
	...
	public float getPlaySpeed(LivingData<?> entitydata) {
		if (entitydata instanceof PlayerData<?>) {
			...
			float correctedSpeed = property.map((value) -> ((PlayerData<?>)entitydata).getAttackSpeed() / value)
					.orElse(this.totalTime * ((PlayerData<?>)entitydata).getAttackSpeed());
	...
	public StaticAnimation loadAnimation(Armature armature, Dist dist) {
		if (this.animationLocation != null) {
			AnimationDataExtractor.extractAttackAnimation(this.animationLocation, this, armature, dist);
		}
