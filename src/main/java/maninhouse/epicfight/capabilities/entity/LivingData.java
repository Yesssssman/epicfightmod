...
public abstract class LivingData<T extends LivingEntity> extends CapabilityEntity<T> {
	...
	public LivingMotion currentOverridenMotion = LivingMotion.IDLE;
	...
	public void onEntityConstructed(T entityIn) {
		...
		if (this.orgEntity.world.isRemote) {
			...
			this.getClientAnimator().playLoopMotion();
		} else {
	...
	public void onEntityJoinWorld(T entityIn) {
		...
		if (CapabilityConfig.CUSTOM_ENTITY_MAP.containsKey(EntityType.getKey(this.orgEntity.getType()))) {
			...
			this.orgEntity.getAttribute(ModAttributes.MAX_STRIKES.get()).setBaseValue(config.getMaxStrikes());
			this.orgEntity.getAttribute(ModAttributes.ARMOR_NEGATION.get()).setBaseValue(config.getArmorNegation());
			this.orgEntity.getAttribute(ModAttributes.IMPACT.get()).setBaseValue(config.getImpact());
		}
	}
	...
	protected void updateOnClient() {
		...
		if (!animator.compareOverridenMotion(this.currentOverridenMotion)) {
			animator.playOverridenLoopMotion();
		}
		
		if (!animator.compareMotion(this.currentMotion)) {
			if (this.state == EntityState.CANCELABLE_POST_DELAY) {
				this.getClientAnimator().getLayer(Layer.Priority.HIGHEST).off(this);
			}
	...
	protected final void commonCreatureUpdateMotion() {
		...
		this.currentOverridenMotion = this.currentMotion;
	}
	
	protected final void commonRangedAttackCreatureUpdateMotion() {
	...
	public float getDamageToEntity(@Nullable Entity targetEntity, @Nullable IExtendedDamageSource source, Hand hand) {
		...
		if (hand == Hand.MAIN_HAND) {
			...
			damage = this.isValidOffhandItem() ? (float) this.orgEntity.getAttributeValue(ModAttributes.OFFHAND_ATTACK_DAMAGE.get()) :
	...
	public float getStunArmor() {
		ModifiableAttributeInstance stunArmor = this.orgEntity.getAttribute(ModAttributes.STUN_ARMOR.get());
	...
	public float getWeight() {
		return (float)this.orgEntity.getAttributeValue(ModAttributes.WEIGHT.get());
	}
	...
	public int getHitEnemies(Hand hand) {
		return (int) (hand == Hand.MAIN_HAND ? this.orgEntity.getAttributeValue(ModAttributes.MAX_STRIKES.get()) : 
			this.isValidOffhandItem() ? this.orgEntity.getAttributeValue(ModAttributes.OFFHAND_MAX_STRIKES.get()) : this.orgEntity.getAttribute(ModAttributes.MAX_STRIKES.get()).getBaseValue());
	}

	public float getArmorNegation(Hand hand) {
		return (float) (hand == Hand.MAIN_HAND ? this.orgEntity.getAttributeValue(ModAttributes.ARMOR_NEGATION.get()) : 
			this.isValidOffhandItem() ? this.orgEntity.getAttributeValue(ModAttributes.OFFHAND_ARMOR_NEGATION.get()) : this.orgEntity.getAttribute(ModAttributes.ARMOR_NEGATION.get()).getBaseValue());
	}

	public float getImpact(Hand hand) {
		return (float) (hand == Hand.MAIN_HAND ? this.orgEntity.getAttributeValue(ModAttributes.IMPACT.get()) : 
			this.isValidOffhandItem() ? this.orgEntity.getAttributeValue(ModAttributes.OFFHAND_IMPACT.get()) : this.orgEntity.getAttribute(ModAttributes.IMPACT.get()).getBaseValue());
	}
