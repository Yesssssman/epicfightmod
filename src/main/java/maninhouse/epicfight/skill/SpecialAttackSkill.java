...
public abstract class SpecialAttackSkill extends Skill {
	...
	protected void generateTooltipforPhase(List<ITextComponent> list, ItemStack itemStack, CapabilityItem cap, PlayerData<?> playerdata, Map<AttackPhaseProperty<?>, Object> propertyMap, String title) {
		...
		double armorNegation = playerdata.getOriginalEntity().getAttribute(ModAttributes.ARMOR_NEGATION.get()).getBaseValue();
		double impact = playerdata.getOriginalEntity().getAttribute(ModAttributes.IMPACT.get()).getBaseValue();
		double maxStrikes = playerdata.getOriginalEntity().getAttribute(ModAttributes.MAX_STRIKES.get()).getBaseValue();
		...
		if (armorNegation != 0.0D) {
			list.add(new TranslationTextComponent(ModAttributes.ARMOR_NEGATION.get().getAttributeName(),
					new StringTextComponent(ItemStack.DECIMALFORMAT.format(armorNegation)).mergeStyle(TextFormatting.GOLD)
			).mergeStyle(TextFormatting.DARK_GRAY));
		}
		
		if (impact != 0.0D) {
			list.add(new TranslationTextComponent(ModAttributes.IMPACT.get().getAttributeName(),
					new StringTextComponent(ItemStack.DECIMALFORMAT.format(impact)).mergeStyle(TextFormatting.AQUA)
			).mergeStyle(TextFormatting.DARK_GRAY));
		}
		
		list.add(new TranslationTextComponent(ModAttributes.MAX_STRIKES.get().getAttributeName(),
