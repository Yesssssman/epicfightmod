...
public class RemoteClientPlayerData<T extends AbstractClientPlayerEntity> extends PlayerData<T> {
	...
	public void updateMotion() {
		...
		if (this.orgEntity.isHandActive() && this.orgEntity.getItemInUseCount() > 0) {
			...
			if (useAction == UseAction.BLOCK || secondUseAction == UseAction.BLOCK) {
				...
				currentOverridenMotion = currentMotion;
		} else {
			...
			if (this.getClientAnimator().prevAiming() && currentOverridenMotion != LivingMotion.AIM) {
	...
	protected void updateOnClient() {
		...
		if (this.swingArm != this.orgEntity.isSwingInProgress) {
			if (!this.swingArm && this.currentMotion != LivingMotion.SLEEP && !this.orgEntity.isActualySwimming() && !this.orgEntity.isElytraFlying()) {
				this.getClientAnimator().playAnimation(Animations.BIPED_DIG, 0.0F);
			} else {
				this.getClientAnimator().playAnimation(Animations.OFF_ANIMATION_MIDDLE, 0.0F);
			}
			this.swingArm = this.orgEntity.isSwingInProgress;
		}
	}
	
	public void onHeldItemChange(CapabilityItem mainHandCap, CapabilityItem offHandCap) {
		this.getClientAnimator().clearOverridenMotions();
		this.getClientAnimator().resetOverridenMotion();
		this.cancelUsingItem();
		
		if (offHandCap != null) {
			Map<LivingMotion, StaticAnimation> motionChanger = offHandCap.getLivingMotionChanges(this);
			if (motionChanger != null) {
				for (Map.Entry<LivingMotion, StaticAnimation> entry : motionChanger.entrySet()) {
					this.getClientAnimator().addOverridenLivingMotion(entry.getKey(), entry.getValue());
				}
			}
		}
		
		if (mainHandCap != null) {
			Map<LivingMotion, StaticAnimation> motionChanger = mainHandCap.getLivingMotionChanges(this);
			if (motionChanger != null) {
				for (Map.Entry<LivingMotion, StaticAnimation> entry : motionChanger.entrySet()) {
					this.getClientAnimator().addOverridenLivingMotion(entry.getKey(), entry.getValue());
				}
