...
public class FirstPersonRenderer extends ArmatureRenderer<ClientPlayerEntity, ClientPlayerData> {
	...
	public void render(ClientPlayerEntity entityIn, ClientPlayerData entitydata, EntityRenderer<ClientPlayerEntity> renderer, IRenderTypeBuffer buffer,
		...
		ClientModel firstModel = entityIn.getSkinType().equals("slim") ? ClientModels.LOGICAL_CLIENT.ENTITY_BIPED_FIRST_PERSON_SLIM :
			ClientModels.LOGICAL_CLIENT.ENTITY_BIPED_FIRST_PERSON_CLASSIC;
		firstModel.draw(matStackIn, buffer.getBuffer(ModRenderTypes.getAnimatedModel(entitydata.getOriginalEntity().getLocationSkin())),
