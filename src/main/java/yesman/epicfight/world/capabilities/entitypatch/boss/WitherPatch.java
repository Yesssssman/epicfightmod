...
public class WitherPatch extends MobPatch<WitherBoss> implements DynamicActionEntity {
	...
	public void initAI() {
		...
		this.original.goalSelector.addGoal(0, new AttackBehaviorGoal(this, MobCombatBehaviors.WITHER_ATTACKS.build(this)));
	}
	
	@Override
	public void initAnimator(ClientAnimator animatorClient) {
		animatorClient.addLivingAnimation(LivingMotion.IDLE, Animations.WITHER_IDLE);
		animatorClient.addLivingAnimation(LivingMotion.DEATH, Animations.WITHER_DEATH);
	}
	...
	public void serverTick(LivingUpdateEvent event) {
		...
		if (this.original.getHealth() <= this.original.getMaxHealth() * 0.5F) {
			...
			if (this.isArmorActivated()) {
				this.original.getEntityData().set(DATA_ARMOR_ACTIVED, false);
			}
		}
	...
	public void activateArmor() {
		this.original.getEntityData().set(DATA_ARMOR_ACTIVED, true);
	}
	...
	public class WitherGhostAttackGoal extends Goal {
		...
		public void start() {
			...
			List<LivingEntity> nearbyEnemies = WitherPatch.this.original.level.getNearbyEntities(LivingEntity.class, WTIHER_TARGETING_CONDITIONS, WitherPatch.this.original, WitherPatch.this.original.getBoundingBox().inflate(20.0D, 5.0D, 20.0D));
			...
			if (--this.summonInverval <= 0) {
				if (this.ghostSummonCount < this.maxGhostSpawn) {
					...
					if (nearbyEnemies.size() > 0) {
						...
						WitherGhostClone ghostclone = new WitherGhostClone((ServerLevel)WitherPatch.this.original.level, summonPosition, WitherPatch.this.original.getTarget());
