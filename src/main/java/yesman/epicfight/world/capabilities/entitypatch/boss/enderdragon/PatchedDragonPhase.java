...
public abstract class PatchedDragonPhase extends AbstractDragonPhaseInstance {
	...
	protected static boolean isInEndSpikes(LivingEntity entity) {
		...
		return blockpos.distSqr(entity.getX(), blockpos.getY(), entity.getZ(), false) < 2000.0D;
	}
	
	protected List<Player> getPlayersWithin(double within) {
		return this.dragon.level.getNearbyPlayers(EnderDragonPatch.NEW_TARGET_TARGETING, this.dragon, this.dragon.getBoundingBox().inflate(within, within, within));
	}
}