...
public class DragonCrystalLinkPhase extends PatchedDragonPhase {
	...
	private float healthRegenPerTick;
	...
	public void begin() {
		...
		this.healthRegenPerTick = (this.dragon.getMaxHealth() - this.dragon.getHealth()) / (this.chargingCount - 2);
		...
		if (this.dragonpatch.isLogicalClient()) {
			...
			if (!this.dragonpatch.isLogicalClient()) {
				int shieldCorrection = this.getPlayersWithin(100.0D).size() - 1;
				this.dragonpatch.setStunShield(STUN_SHIELD_AMOUNT + 15.0F * shieldCorrection);
			}
		}
	}
	...
	public void doServerTick() {
		...
		if (this.chargingCount > 0) {
			this.dragon.setHealth(this.dragon.getHealth() + this.healthRegenPerTick);
		}
	}
