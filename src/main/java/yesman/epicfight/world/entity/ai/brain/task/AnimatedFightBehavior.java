...
public class AnimatedFightBehavior<T extends MobPatch<?>> extends Behavior<Mob> {
	protected final T mobpatch;
	protected final CombatBehaviors<T> combatBehaviors;
	
	public AnimatedFightBehavior(T mobpatch, CombatBehaviors<T> combatBehaviors) {
	...
	protected boolean canStillUse(ServerLevel levelIn, Mob entityIn, long gameTimeIn) {
		return this.checkExtraStartConditions(levelIn, entityIn) && BehaviorUtils.canSee(entityIn, this.getAttackTarget(entityIn)) && !this.mobpatch.getEntityState().hurt();
	}
