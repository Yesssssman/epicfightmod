	...
	public void onInitiate(SkillContainer container) {
		...
		listener.addEventListener(EventType.MOVEMENT_INPUT_EVENT, EVENT_UUID, (event) -> {
			...
			if (jumpPressed && !jumpPressedPrev) {
				...
				if (jumpCounter > 0 || !event.getPlayerPatch().getOriginal().onGround()) {
					if (jumpCounter < (this.extraJumps + 1)) {
						...
						float pulse = Mth.clamp(0.3F + EnchantmentHelper.getSneakingSpeedBonus(event.getPlayerPatch().getOriginal()), 0.0F, 1.0F);
