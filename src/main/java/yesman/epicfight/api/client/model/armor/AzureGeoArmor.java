...
public class AzureGeoArmor extends ArmorModelTransformer {
	...
	public static void getGeoArmorTexturePath(AnimatedArmorTextureEvent event) {
		IItemRenderProperties customRenderProperties = (IItemRenderProperties) event.getItemstack().getItem().getRenderPropertiesInternal();
		
		if (customRenderProperties != null) {
			HumanoidModel<?> extensionRenderer = customRenderProperties.getArmorModel(event.getLivingEntity(), event.getItemstack(), event.getEquipmentSlot(), event.getOriginalModel());
