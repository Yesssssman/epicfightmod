...
public class OBBCollider extends Collider {
	...
	private static boolean collisionDetection(Vec3 seperateAxis, Vec3 toOpponent, OBBCollider box1, OBBCollider box2) {
		...
		distance = seperateAxis.dot(toOpponent) > 0.0F ? toOpponent : new Vec3(-toOpponent.x, -toOpponent.y, -toOpponent.z);
		
		for (Vec3 vertexVector : box1.rotatedVertex) {
			Vec3 temp = seperateAxis.dot(vertexVector) > 0.0F ? vertexVector : new Vec3(-vertexVector.x, -vertexVector.y, -vertexVector.z);
	...
	private static float getProjectedScale(Vec3 normal, Vec3 projecting) {
		double dot = normal.dot(projecting);
		double normalScale = 1 / ((normal.x * normal.x) + (normal.y * normal.y) + (normal.z * normal.z));
		Vec3 projVec = new Vec3(dot * normal.x * normalScale, dot * normal.y * normalScale, dot * normal.z * normalScale);
