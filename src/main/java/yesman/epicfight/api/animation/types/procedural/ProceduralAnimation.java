...
public interface ProceduralAnimation {
	...
	default Vec3f getRayCastedTipPosition(Vec3f clipStart, OpenMatrix4f toWorldCoord, EnderDragonPatch enderdragonpatch, float maxYDown, float leastHeight) {
		...
		BlockRayTraceResult clipResult = enderdragonpatch.getOriginal().level.clip(new RayTraceContext(new Vector3d(clipStartWorld.x, clipStartWorld.y, clipStartWorld.z),
				new Vector3d(clipStartWorld.x, clipStartWorld.y - maxYDown, clipStartWorld.z), RayTraceContext.BlockMode.COLLIDER, RayTraceContext.FluidMode.NONE, enderdragonpatch.getOriginal()));
		float dy = (clipResult.getType() != RayTraceResult.Type.MISS) ? (float)clipStartWorld.y - clipResult.getBlockPos().getY() - 1 : maxYDown;
