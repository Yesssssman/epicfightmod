...
public class EnderDraonWalkAnimation extends StaticAnimation implements ProceduralAnimation {
	...
	public EnderDraonWalkAnimation(float convertTime, String path, Model model, IKSetter[] ikSetters) {
	...
	public void loadAnimation(ResourceManager resourceManager) {
		...
		this.setIKData(this.ikSetters, this.getTransfroms(), this.tipPointTransform, this.getModel().getArmature(), false, false);
	}
	...
	public Pose getPoseByTime(LivingEntityPatch<?> entitypatch, float time, float partialTicks) {
		...
		if (entitypatch instanceof EnderDragonPatch) {
	    	...
	    	for (IKSetter ikSetter : this.ikSetters) {
	    		for (String jointName : ikSetter.pathToEndJoint) {
					pose.putJointData(jointName, this.jointTransforms.get(jointName).getKeyframes()[ikSetter.ikPose].transform().copy());
				}
	...
	public void tick(LivingEntityPatch<?> entitypatch) {
		...
		if (entitypatch instanceof EnderDragonPatch) {
			...
			if (!(enderdragonpatch.getAnimator().getPlayerFor(null).getPlay() == this)) {
				return;
			}
			...
			for (IKSetter ikSetter : this.ikSetters) {
				...
				TipPointAnimation opponentTipAnim = enderdragonpatch.getTipPointAnimation(ikSetter.opponentJoint);
