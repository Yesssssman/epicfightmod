...
public class EnderDragonDynamicActionAnimation extends ActionAnimation implements ProceduralAnimation {
	...
	public void begin(LivingEntityPatch<?> entitypatch) {
		...
		if (entitypatch instanceof EnderDragonPatch) {
			...
			TransformSheet dynamicRootTransform = enderdragonpatch.getRootDynamicTransform();
			...
			for (IKSetter ikSetter : this.ikSetters) {
				...
				Vec3f startpos = dynamicRootTransform.getInterpolatedTranslation(0.0F);
				
				for (int i = 0; i < keyframes.length; i++) {
					Keyframe kf = keyframes[i];
					Vec3f dynamicpos = dynamicRootTransform.getInterpolatedTranslation(kf.time()).sub(startpos);
