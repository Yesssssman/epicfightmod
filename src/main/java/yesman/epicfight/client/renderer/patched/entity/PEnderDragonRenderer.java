...
@OnlyIn(Dist.CLIENT)
public class PEnderDragonRenderer extends PatchedEntityRenderer<EnderDragonEntity, EnderDragonPatch, EnderDragonRenderer> {
	...
	public void render(EnderDragonEntity entityIn, EnderDragonPatch entitypatch, EnderDragonRenderer renderer, IRenderTypeBuffer buffer, MatrixStack poseStack, int packedLight, float partialTicks) {
		...
		if (entityIn.nearestCrystal != null) {
			float x = (float)(entityIn.nearestCrystal.getX() - MathHelper.lerp(partialTicks, entityIn.xo, entityIn.getX()));
	        float y = (float)(entityIn.nearestCrystal.getY() - MathHelper.lerp(partialTicks, entityIn.yo, entityIn.getY()));
	        float z = (float)(entityIn.nearestCrystal.getZ() - MathHelper.lerp(partialTicks, entityIn.zo, entityIn.getZ()));
	        poseStack.pushPose();
			EnderDragonRenderer.renderCrystalBeams(x, y + EnderCrystalRenderer.getY(entityIn.nearestCrystal, partialTicks), z, partialTicks, entityIn.tickCount, poseStack, buffer, packedLight);
			poseStack.popPose();
		}
		
		if (entityIn.dragonDeathTime > 0) {
			float deathTimeProgression = ((float) entityIn.dragonDeathTime + partialTicks) / 200.0F;
			IVertexBuilder lightningBuffer = buffer.getBuffer(RenderType.lightning());
	...
	public void mulPoseStack(MatrixStack matStack, Armature armature, EnderDragonEntity entityIn, EnderDragonPatch entitypatch, float partialTicks) {
		...
		if (!entitypatch.isGroundPhase() || entitypatch.getOriginal().dragonDeathTime > 0) {
		    ...
		    float f2 = entitypatch.getOriginal().dragonDeathTime > 0 ? 0.0F : (float)MathHelper.rotWrap((entityIn.getLatencyPos(5, partialTicks)[0] - entityIn.getLatencyPos(10, partialTicks)[0]));
	...
	protected int getOverlayCoord(EnderDragonEntity entity, EnderDragonPatch entitypatch, float partialTicks) {
		IPhase currentPhase = entity.getPhaseManager().getCurrentPhase();
