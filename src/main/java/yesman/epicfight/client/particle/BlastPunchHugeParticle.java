...
@OnlyIn(Dist.CLIENT)
public class BlastPunchHugeParticle extends AnimatedMeshParticle {
	protected BlastPunchHugeParticle(ClientWorld worldIn, double xCoordIn, double yCoordIn, double zCoordIn, double xSpeedIn,
	...
	public static class Factory implements IParticleFactory<BasicParticleType> {
		...
		public Particle makeParticle(BasicParticleType typeIn, ClientWorld worldIn, double x, double y, double z, double xSpeed, double ySpeed, double zSpeed) {
			//if(mesh == null) mesh = Minecraft.getInstance().getModelManager().getModel(new ModelResourceLocation(
					//ForgeRegistries.PARTICLE_TYPES.getKey(Particles.BLAST_PUNCH_HUGE.get()), "particle"));
			
			return new BlastPunchHugeParticle(worldIn, x, y, z, xSpeed, ySpeed, zSpeed, mesh);
		}
	}
