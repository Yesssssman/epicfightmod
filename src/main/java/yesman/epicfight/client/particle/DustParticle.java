			...
			for (int vx = -1; vx <= 1; vx+=2) {
				for (int vz = -1; vz <= 1; vz += 2) {
					for (int i = 0; i < density; i++) {
						...
						level.addParticle(EpicFightParticles.EXPANSIVE_DUST.get(), x, y, z, rand.x, rand.y, rand.z);
					}
				}
			}
		}
				...
				for (int y = -1; y <= 1; y +=2) {
					for (int z = -1; z <= 1; z += 2) {
						for (int i = 0; i < this.density; i++) {
							...
							this.level.addParticle(EpicFightParticles.CONTRACTIVE_DUST.get(), this.x + rand.x, this.y + rand.y, this.z + rand.z, -rand.x, -rand.y, -rand.z);
						}
					}
				}
			}
