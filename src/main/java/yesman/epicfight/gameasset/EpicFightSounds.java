...
public class EpicFightSounds {
    ...
    private static void registerSoundsWithHelper(RegisterEvent.RegisterHelper<SoundEvent> helper, SoundEvent... soundEvents) {
        for (SoundEvent soundEvent : soundEvents)
            helper.register(soundEvent.getLocation(), soundEvent);
    }

    public static void registerSoundRegistry(final RegisterEvent event) {
        event.register(ForgeRegistries.Keys.SOUND_EVENTS,
                helper -> registerSoundsWithHelper(helper,
						BLADE_HIT,
						BLUNT_HIT,
						BLUNT_HIT_HARD,
						CLASH,
